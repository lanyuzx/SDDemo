...
open class JXSegmentedView: UIView, JXSegmentedViewRTLCompatible {
    ...
    ///额外增加 选项默认背景色 背景色作用于cell的backview上
    open var itemBackgroundColor: UIColor?
    ///额外增加 选项选中背景色 背景色作用于cell的backview上
    open var itemSelectBackgroundColor: UIColor?
    ...
    private func selectItemAt(index: Int, selectedType: JXSegmentedViewItemSelectedType) {
        ...
        //额外增加
        if self.itemBackgroundColor != nil {
            (currentSelectedCell as? JXSegmentedTitleCell)?.backViewColor = self.itemBackgroundColor
        }
        if self.itemSelectBackgroundColor != nil {
            (willSelectedCell as? JXSegmentedTitleCell)?.backViewColor = self.itemSelectBackgroundColor
        }
        //额外增加
...
extension JXSegmentedView: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = dataSource?.segmentedView(self, cellForItemAt: indexPath.item) {
            ...
            // 额外增加
            if self.itemBackgroundColor != nil && self.itemSelectBackgroundColor != nil {
                (cell as? JXSegmentedTitleCell)?.backViewColor = (indexPath.row == self.selectedIndex) ? self.itemSelectBackgroundColor : self.itemBackgroundColor
            }
            // 额外增加
            return cell
        }else {
